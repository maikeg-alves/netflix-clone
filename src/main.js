...
const key = () => {
  return `?api_key=bca1246da75f747a821602efcbca943d&language&language=pt-BR`;
};
...
Api(
  `/discover/movie/${key()}&sort_by=popularity.desc&page=${randomPages()}&year=2022`
).then((resposta) => RandomBaner(resposta.data.results));

Api(`/discover/movie/${key()}&sort_by=popularity.desc&page=1&year=2022`).then(
  (resposta) => moviespage1(resposta.data.results)
);

Api(`/discover/movie/${key()}&sort_by=popularity.desc&page=2&year=2022`).then(
  (resposta) => moviespage2(resposta.data.results)
);

Api(`/discover/movie/${key()}&sort_by=popularity.desc&page=3&year=2022`).then(
  (resposta) => moviespage3(resposta.data.results)
);

Api(`/discover/movie/${key()}&sort_by=popularity.desc&page=4&year=2022`).then(
  (resposta) => moviespage4(resposta.data.results)
);
...
const universalBanerSelect = (id, grede, close) => {
  Api(`/movie/${id + key()}`).then((resposta) => selectmovie([resposta.data]));
